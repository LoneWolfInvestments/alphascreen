...
function signalFromBreadth(breadth) {
  if (breadth == null) return { label: "No data", color: "#8b93a7" };
  if (breadth > 5) return { label: "Accumulating", color: "#3ecf8e" };
  if (breadth < -5) return { label: "Distributing", color: "#f0555a" };
  return { label: "Flat", color: "#f0b955" };
}
...
export default function FundamentalScreener() {
  ...
  const sorted = [...rows].sort((a, b) => (b.inst_net_breadth || 0) - (a.inst_net_breadth || 0));
  ...
  return (
    <div className="panel">
      ...
      <p className="disclaimer" style={{ marginTop: 0, marginBottom: 12, borderTop: "none", paddingTop: 0 }}>
        Adding a ticker here just registers it so the automations have something to update — everything below fills
        in automatically from there.
      </p>
      ...
      <div className="table-wrap">
        <table>
          <thead>
            <tr>
              <th>Ticker</th><th>Inst. Buyers</th><th>Inst. Sellers</th><th>Net Breadth</th><th>Read</th>
          ...
          <tbody>
            {sorted.map((r) => {
              const sig = signalFromBreadth(r.inst_net_breadth);
              return (
                <tr key={r.ticker}>
                  <td><strong>{r.ticker}</strong></td>
                  <td>{r.inst_buyers ?? "—"}</td>
                  <td>{r.inst_sellers ?? "—"}</td>
                  <td>{r.inst_net_breadth != null ? `${r.inst_net_breadth > 0 ? "+" : ""}${r.inst_net_breadth.toFixed(1)}%` : "—"}</td>
      ...
      <p className="disclaimer">
        Inst. Buyers/Sellers are the count of institutional holders who increased/opened vs. decreased/closed their
        position last quarter (13F-derived, via Nasdaq's public feed). Net Breadth is (Buyers − Sellers) ÷ Total,
        shown as a raw percentage rather than compressed into an arbitrary score — read the actual number and judge
        magnitude yourself. P/E, Market Cap, EPS, Margin, Next Earnings, and Analyst rating are pulled automatically
        from Finnhub.
      </p>
    </div>
  );
